...
DATATYPE* RTREE_QUAL::KNN(ELEMTYPE queryPoint[NUMDIMS], int k)
{
	...
	ASSERT(knnList != NULL);
	int i;
	double max = (numeric_limits<double>::max)();   //����ȡ��ֵ
	for(i = 0; i < k; i++)
	{
...
void RTREE_QUAL::KNearNeighbour(ELEMTYPE queryPoint[NUMDIMS], Node* a_node, int k, NeighbourNode* knnList, double& ndistance)
{
	...
	if(a_node->IsInternalNode()) // This is an internal node in the tree
	{
		Branch activebranch[MAXNODES];   //�Խڵ㰴��minMax��������,������ndistance��ֱ���׵�
		...
		for(index = 0; index < activeCount; index ++)
		{
			...
			if(mindist >=ndistance)  //������ֱ�Ӽ�֦
				break;
			KNearNeighbour(queryPoint, activebranch[index].m_child, k, knnList, ndistance);  //�ݹ�
		}
	}
	else // This is a leaf node
	{
		int i, index, j;
		for(index=0; index < a_node->m_count; ++index)  //�Խڵ��е�ÿһ��������ֵ�Ƚ�
		{
			double dist = 1.0;
			ELEMTYPE* point;
			...
			for(i = 0; i < NUMDIMS; i++)
			{
				dist +=  ((point[i] - queryPoint[i])/scale) * ((point[i] - queryPoint[i])/scale);  //�����Է�Χ��������С�Ĵ���
			}
			if(dist >= ndistance)     //ֱ�Ӳ��Ƚϣ����������ƶ�һ����k+1���㣬ֻ������ȡk��
				continue;
			//��������
			j = k - 1;
			while(j>0)
